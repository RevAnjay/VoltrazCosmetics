...
import xyz.voltraz.cosmetics.events.*;
...
import xyz.voltraz.cosmetics.provider.ItemsAdder;
...
public class PlayerData {
    ...
    public static PlayerData getPlayer(OfflinePlayer player){
        return players.computeIfAbsent(player.getUniqueId(), uuid ->
            new PlayerData(uuid, player.getName(), null/*VoltrazCosmetics.getInstance().getVersion().createRangeManager(player.getPlayer())*/)
        );
    }
    ...
    public int getCosmeticCount(CosmeticType cosmeticType){
        int i = 0;
        if(VoltrazCosmetics.getInstance().isPermissions()){
    ...
    public void clearPreviewHat(){
        ...
        NPC npc = VoltrazCosmetics.getInstance().getVersion().getNPC(player);
        if(npc == null) return;
    ...
    public void saveItems(){
        if(!VoltrazCosmetics.getInstance().isZoneHideItems()) return;
        Player player = getOfflinePlayer().getPlayer();
    ...
    public void verifyWorldBlackList(VoltrazCosmetics plugin) {
