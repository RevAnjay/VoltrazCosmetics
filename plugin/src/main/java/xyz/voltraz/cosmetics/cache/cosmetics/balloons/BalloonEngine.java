...
public class BalloonEngine {
    ...
    public void setStatePlayOn(int state){
        ...
        switch (state){
            case 0:
                ...
                break;
            case 1:
                if(!VoltrazCosmetics.getInstance().getModelEngine().existAnimation(modelId, walk_animation)) return;
    ...
    public Set<String> getBones() {
        ...
        return VoltrazCosmetics.getInstance().getModelEngine().getAllBonesIds(balloonUniqueId, modelId);
    }

    public ArmorStand spawnModel(Location location){
        ArmorStand armorStand = VoltrazCosmetics.getInstance().getVersion().spawnArmorStand(location);
        balloonUniqueId = VoltrazCosmetics.getInstance().getModelEngine().spawnModel(armorStand, modelId, location, offsetModel);
        return armorStand;
    }

    public void updateTeleport(LivingEntity leashed, Location location) {
        if(balloonUniqueId == null) return;
        VoltrazCosmetics.getInstance().getVersion().updatePositionFakeEntity(leashed, location);
        Set<Player> players = VoltrazCosmetics.getInstance().getModelEngine().getTrackedPlayers(balloonUniqueId);
        VoltrazCosmetics.getInstance().getVersion().teleportFakeEntity(leashed, players);
        VoltrazCosmetics.getInstance().getModelEngine().movementModel(balloonUniqueId, location);
    }
    ...
    public void showModel(Player player) {
        VoltrazCosmetics.getInstance().getModelEngine().showModel(balloonUniqueId, player);
    }

    public void hideModel(Player player) {
        VoltrazCosmetics.getInstance().getModelEngine().hideModel(balloonUniqueId, player);
    }
    ...
    public void spawnLeash(Entity entity) {
        if(balloonUniqueId == null) return;
        VoltrazCosmetics.getInstance().getModelEngine().spawnLeash(entity, balloonUniqueId, modelId);
    }
}
