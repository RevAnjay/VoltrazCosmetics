...
import xyz.voltraz.cosmetics.cache.Token;
import xyz.voltraz.cosmetics.cache.inventories.*;
    ...
    public void setItems() {
        ...
        if(!cosmetics.isEmpty()) {
            ...
            for (int i = 0; i < getMaxItemsPerPage(); i++) {
                ...
                Items items = new Items(getPage()+index+"_balloon", Items.getItem("balloon-template").copyItem(playerData, cosmetic, cosmetic.getItemStack()));
                SlotMenu slotMenu;
                ...
                if(playerData.getBalloon() != null){
                    if(playerData.getBalloon().getId().equalsIgnoreCase(cosmetic.getId())){
