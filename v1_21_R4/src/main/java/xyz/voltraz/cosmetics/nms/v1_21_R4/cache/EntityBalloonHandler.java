package xyz.voltraz.cosmetics.nms.v1_21_R4.cache;
...
public class EntityBalloonHandler extends EntityBalloon {
    ...
    public void updateBigHead(){
        ...
        if(distSq > SQUARED_WALKING){
            ...
            tempStandTo.setDirection(tempStandDir);
            tempStandTo.add(tempDistVec);
            leashed.setPos(tempStandTo.getX(), tempStandTo.getY(), tempStandTo.getZ()); leashed.forceSetRotation(tempStandTo.getYaw(), tempStandTo.getPitch());
            armorStand.setPos(tempStandTo.getX(), tempStandTo.getY() - 1.3, tempStandTo.getZ()); armorStand.forceSetRotation(tempStandTo.getYaw(), tempStandTo.getPitch());
        }else {
            if (tempStandDir.lengthSquared() > 0) {
                tempStandDir.normalize();
            }
