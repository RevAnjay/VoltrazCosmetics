...
public class EntityBalloonHandler extends EntityBalloon {
    ...
    public void updateBigHead(){
        ...
        if(distance1.distanceSquared(distance2) > SQUARED_WALKING){
            ...
            Location standTo = stand.clone().setDirection(standDir.setY(0)).add(distVec.clone());
            Location newLocation = standTo.clone();
            leashed.setPos(newLocation.getX(), newLocation.getY(), newLocation.getZ()); leashed.forceSetRotation(newLocation.getYaw(), newLocation.getPitch());;
            armorStand.setPos(newLocation.getX(), newLocation.getY() - 1.3, newLocation.getZ()); armorStand.forceSetRotation(newLocation.getYaw(), newLocation.getPitch());;
        }else {
            if (!standDir.equals(new Vector())) {
                standDir.normalize();
            }
