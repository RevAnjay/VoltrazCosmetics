...
public class NPCHandler extends NPC {
    ...
    public void addNPC(Player player, Location location) {
        ...
        ReflectionUtils.absMoveTo(npc, location.getX(), location.getY(), location.getZ(), location.getYaw(), 0);
        //balloon
        ...
        ReflectionUtils.absMoveTo(entityPunch, player.getLocation().getX(), player.getLocation().getY(), player.getLocation().getZ(), player.getLocation().getYaw(), player.getLocation().getPitch());
        ...
        try {
            ...
            npc.getBukkitEntity().getProfile().properties().put("textures", new Property("textures", skin[0], skin[1]));
    ...
    public void spawnNPC(Player player) {
        ...
        }.runTaskLater(Objects.requireNonNull(Bukkit.getPluginManager().getPlugin("VoltrazCosmetics")), 20L);
        addPassenger(player);
    }
    ...
    public void balloonNPC(Player player, Location location, ItemStack itemStack, boolean bigHead){
        ...
        ReflectionUtils.absMoveTo(balloon, balloonPos.getX(), balloonPos.getY(), balloonPos.getZ(), balloonPos.getYaw(), balloonPos.getPitch());
        ...
        if(isBigHead()){
            balloon.setHeadPose(new Rotations(ReflectionUtils.rotX(balloon.getHeadPose()), 0, 0));
        }
    ...
    public void animation(Player player){
        ...
        if (!rotateLoop) {
            rot += 0.01;
            balloon.setBodyPose(new Rotations(ReflectionUtils.rotX(balloon.getBodyPose()) - 0.5f, ReflectionUtils.rotY(balloon.getBodyPose()), ReflectionUtils.rotZ(balloon.getBodyPose()) + rotate));
            if (rot > 0.20) {
                rotateLoop = true;
            }
        } else {
        ...
        ReflectionUtils.absMoveTo(leashed, balloonPosition.getX(), balloonPosition.getY(), balloonPosition.getZ(), balloonPosition.getYaw(), balloonPosition.getPitch());
        ReflectionUtils.absMoveTo(balloon, balloonPosition.getX(), balloonPosition.getY() - 1.3, balloonPosition.getZ(), balloonPosition.getYaw(), balloonPosition.getPitch());
        ...
        p.connection.send(ReflectionUtils.createTeleportPacket(balloon));
    }
    ...
    public void animationBigHead(Player player){
        ...
        if (!rotateLoop) {
            rot += 0.01;
            balloon.setHeadPose(new Rotations(ReflectionUtils.rotX(balloon.getHeadPose()) - 0.5f, ReflectionUtils.rotY(balloon.getHeadPose()), ReflectionUtils.rotZ(balloon.getHeadPose()) + rotate));
            if (rot > 0.20) {
                rotateLoop = true;
            }
        } else {
